...
namespace assignment2
{
	...
	void TestWiki1()
	{
		...
		for (size_t i = 0; i < MAX_CAPACITY + 10; i++)
		{
			...
			std::cout << MAX_SPEED_LABLE << air->GetMaxSpeed() << std::endl
				<< CUR_P_LABLE << air->GetPassengersCount() << std::endl;
		}

		while (air->RemovePassenger(0))
		{
			std::cout << CUR_P_LABLE << air->GetPassengersCount() << std::endl;;
		}
		...
		{
			Vehicle* tempVPointer = d->GetVehicleOrNull(i);
